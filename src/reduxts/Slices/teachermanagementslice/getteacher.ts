...
export const Teacherdata = createAsyncThunk('api/TeacherData', async (id) => {
...
const apiSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(Teacherdata.pending, (state) => {
        state.loading = true;
      })
      .addCase(Teacherdata.fulfilled, (state, action) => {
       // console.log("reducer from slice example", action);
        state.loading = false;
        state.data = action.payload;
        state.error = null;
      })
      .addCase(Teacherdata.rejected, (state, action) => {
        state.loading = false;
